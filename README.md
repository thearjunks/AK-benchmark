...
For the Lighthouse fallback, add `LIGHTHOUSE_CALLBACK_TOKEN` as a GitHub Actions repository secret, then add the same value plus `GITHUB_ACTIONS_TOKEN` to Hostinger. The workflow runs Chrome outside Hostinger for any failed website and returns a signed result. If the callback is blocked, the server retrieves and verifies the signed GitHub artifact instead. Mobile and Desktop are retried independently, and the server combines successful PageSpeed and Lighthouse device results. Same-domain redirects are validated; Zain's exact `/en/shop?error=login_required` audit redirect is accepted because automated Chrome reports `/en/shop` as the displayed page and returns all four categories.

Local development can opt into the same remote worker with `LOCAL_GITHUB_FALLBACK=true` in the untracked `.env.local`. It uses `GITHUB_ACTIONS_TOKEN` when configured, otherwise the PC's existing non-interactive GitHub credential-helper sign-in. No token is printed or written by the application. Local results are authenticated through GitHub and checked against the requested branch, unique request ID, and URL; production still requires the HMAC signature. The local flag has no effect in production mode. The worker always uses its approved production callback address, not localhost. An unavailable external audit is shown as unavailable, not as an actual zero score.
