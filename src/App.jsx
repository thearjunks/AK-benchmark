...
import {
  ...
  History, LockKeyhole, LogOut, Mail, Monitor, Plus, Presentation, RefreshCw, Search, Send, ShieldCheck, Smartphone, Sparkles, Trash2, UserCheck, UserPlus, Users, XCircle
} from 'lucide-react'
...
function AdminAccessScreen() {
  ...
  const sections = [['overview', 'Benchmark overview'], ['history', 'Score history'], ['ppt', 'PPT Report'], ['findings', 'Audit findings'], ['emails', 'Emails to send']]
  ...
  async function decide(request, decision) {
    const response = await fetch(`/api/admin/access-requests/${request.id}/decision`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ decision, role: 'user', sections: ['overview', 'history', 'ppt', 'findings'], canSendEmail: false, canDownload: true }) })
    const result = await response.json().catch(() => ({}))
...
function DashboardApp({ currentUser, permissions, onLogout }) {
  ...
  const initialView = ['overview', 'history', 'ppt', 'findings', 'emails', 'admin'].find(section => allowedSections.includes(section)) || 'overview'
  ...
  return <div className="matrix-app">
    <aside className="dashboard-sidebar">
      ...
      <nav aria-label="Dashboard screens">
        ...
        {allowedSections.includes('ppt') && <button className={view === 'ppt' ? 'active' : ''} onClick={() => setView('ppt')}><Presentation size={18}/><span>PPT Report</span></button>}
    ...
    <header className="matrix-topbar">
      <div className="page-context"><span>STC Kuwait digital intelligence</span><strong>{view === 'overview' ? 'Benchmark overview' : view === 'history' ? 'Score history' : view === 'ppt' ? 'PPT Report' : view === 'findings' ? 'Audit findings' : view === 'admin' ? 'Access management' : 'Emails to send'}</strong></div>
    ...
    <main className="matrix-main">
      ...
      <div className={`view-screen ppt-screen ${view === 'ppt' ? 'active' : ''}`}>
        {allowedSections.includes('ppt') && <PptReport history={history} domains={historyDomains} labels={websiteLabels} canDownload={permissions.canDownload} notify={notify}/>}
      </div>
