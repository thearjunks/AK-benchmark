...
function TrendChart({ site }) {
  ...
  return <svg className="ppt-trend-svg" viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`${site.label} Mobile and Desktop performance trend`}>
    ...
    {site.mobile.map((value,index) => Number.isFinite(value) ? <g key={`md-${index}`}><circle className="mobile-dot" cx={point(index,value).x} cy={point(index,value).y} r="5"><title>{site.labels[index]} Mobile: {valueLabel(value)}</title></circle><text className="mobile-value" x={point(index,value).x} y={point(index,value).y + 20} textAnchor="middle">{Math.round(value)}</text></g> : null)}
    {site.desktop.map((value,index) => Number.isFinite(value) ? <g key={`dd-${index}`}><circle className="desktop-dot" cx={point(index,value).x} cy={point(index,value).y} r="5"><title>{site.labels[index]} Desktop: {valueLabel(value)}</title></circle><text className="desktop-value" x={point(index,value).x} y={point(index,value).y - 10} textAnchor="middle">{Math.round(value)}</text></g> : null)}
...
export default function PptReport({ history, domains, labels, canDownload, notify }) {
  ...
  return <div className="ppt-report">
    <section className="ppt-report-hero"><div><span>Management reporting</span><h1>PPT performance dashboard</h1><p>Select a reporting period, review the same visuals that will appear in the management presentation, and download the completed PowerPoint.</p></div><button onClick={download} disabled={!canDownload || downloading || !model.records.length}><Download size={17}/>{downloading ? 'Building PPT…' : 'Download PPT Report'}</button></section>
    ...
    {site && model.records.length ? <>
      <section className="ppt-snapshot">
        ...
        <div className="ppt-kpis"><article><span>Mobile period average</span><strong className="mobile">{valueLabel(site.kpis.mobileAverage)}</strong><small>{signed(site.kpis.mobileChange)} pts from first to latest</small></article><article><span>Desktop period average</span><strong className="desktop">{valueLabel(site.kpis.desktopAverage)}</strong><small>{signed(site.kpis.desktopChange)} pts from first to latest</small></article><article><span>Latest reading</span><strong>{valueLabel(site.kpis.latestMobile)} / {valueLabel(site.kpis.latestDesktop)}</strong><small>Mobile / Desktop{site.latestLabel ? ` on ${site.latestLabel}` : ''}</small></article><article><span>Overall change</span><strong>{signed(site.kpis.mobileChange)} / {signed(site.kpis.desktopChange)}</strong><small>Mobile / Desktop across period</small></article></div>
