...
set "API_URL=%APP_URL%api/automation-state"

cd /d "%APP_DIR%"

powershell.exe -NoProfile -Command "try { if ((Invoke-WebRequest -UseBasicParsing -TimeoutSec 3 -Uri '%API_URL%').StatusCode -eq 200) { exit 0 } } catch {}; exit 1"
...
for /l %%I in (1,1,30) do (
  powershell.exe -NoProfile -Command "try { if ((Invoke-WebRequest -UseBasicParsing -TimeoutSec 2 -Uri '%API_URL%').StatusCode -eq 200) { exit 0 } } catch {}; exit 1"
  if not errorlevel 1 goto open_app
